...
import { BriefcaseBusiness, FileText, Folder, Home, Mail, MessageCircle } from "lucide-react";
...
export function Sidebar({ showCloseButton = true }: { showCloseButton?: boolean } = {}) {
  ...
  return (
    <aside className="flex h-full w-full flex-col overflow-hidden border-r border-border bg-surface">
      <div className="shrink-0 border-b border-border p-2">
        <div className="flex items-start justify-between gap-1">
          <div className="flex min-w-0 flex-1 items-center gap-2 px-2 py-1.5 text-[15px] font-semibold text-foreground">
            <MessageCircle size={16} aria-hidden className="shrink-0" />
        ...
        <button
          type="button"
          onClick={() => window.dispatchEvent(new Event("collabai:open-command-palette"))}
          className="mt-1 flex w-full items-center justify-between rounded-md border border-border bg-background px-3 py-2 text-sm text-secondary transition-colors hover:bg-hover hover:text-foreground"
        >
          <span>Actions</span>
